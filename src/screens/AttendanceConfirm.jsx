...
export default function AttendanceConfirm({ eventId, onDone }) {
  ...
  async function load() {
    ...
    const { data: att } = await supabase
      ...
      .select('user_id, guest_confirmed_attended, profiles(first_name, last_name)')
      .eq('event_id', eventId);
    const list = att || [];
    setAttendees(list);
    const initial = {};
  ...
  async function handleConfirm() {
    ...
    for (const a of attendees) {
      ...
      if (attended) {
        anyoneAttended = true;
        // Засчитываем гостю встречу в журнал статистики только если он САМ тоже
        // подтвердил, что был на встрече — так ни организатор, ни гость не могут
        // в одиночку исказить чужую статистику. Если гость ещё не ответил —
        // подождём: досчитается позже, когда он сам подтвердит явку.
        if (a.guest_confirmed_attended === true) {
    ...
    <div className="screen" style={{ overflowY: 'auto' }}>
      <div style={{ padding: '20px 20px 30px' }}>
        ...
        <div style={{
          ...
          Каждый гость тоже подтвердит явку сам — в статистику попадёт только то, в чём вы сойдётесь.
        </div>
